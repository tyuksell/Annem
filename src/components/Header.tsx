...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#fcfaf7]/85 backdrop-blur-xl border-b border-[#e8e3d8]/80 shadow-[0_4px_24px_-4px_rgba(0,0,0,0.04)] transition-all">
      <div className="max-w-7xl lg:max-w-5xl mx-auto px-3 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center space-x-1.5 sm:space-x-3 shrink-0">
          ...
          {userProfile.isLoggedIn ? (
            <button
              ...
              className={`w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-gradient-to-br from-[#3d5a45] via-[#4a6b54] to-[#f07052] text-white flex items-center justify-center font-extrabold text-sm sm:text-base shadow-xs hover:shadow-md transition-all border border-white/40 relative shrink-0 active:scale-95 cursor-pointer ${
                currentTab === 'profile' ? 'ring-2 ring-[#3d5a45]/30' : ''
              }`}
